...
int main(int argc, char* argv[]) {
  ...
  */
  ...
  double * state = alloc_params();
  read_params(state,paramsname,"r");
  if (debug) {
    print_params(state,0,stdout);
    printf("\n");
  }
  double * candidate = alloc_params();

  double * statistics = alloc_params();
  copy_params(statistics,state);

  model_rng* stan_model = construct(dataname, seed, 0);
  if (!stan_model) {
    return 1;
  }

  printf("This model's name is %s.\n", name(stan_model));
  printf("It has %d parameters.\n", param_num(stan_model, 0, 0));
  printf("Info: %s\n", model_info(stan_model));
  ...
  for (int i = 0; i < n; ++i) {
    ...
    double lp_parameters;
    log_density(stan_model,1,1,state, &lp_parameters);
    ...
    if (debug) {
      print_params(candidate,0,stdout);
      printf("\n");
    }

    double lp_candidate;
    log_density(stan_model,1,1,candidate,&lp_candidate);
	...
	printf("keeping sample\n");
      ...
      print_params(statistics,1,output);
      fprintf(output,"\n");
    }
  }
